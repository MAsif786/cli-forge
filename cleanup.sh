#!/bin/bash
# Mac Storage Cleanup Script
# Scans for reclaimable space, shows large chunks, asks for approval.

set -e

# Force line-buffered output so progress shows immediately
exec 2>&1
if test -t 1; then true; else exec > >(cat); fi
...
scan_docker() {
  ...
  # Store reclaim value for later use
