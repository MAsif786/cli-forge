...
async function main() {
  intro(chalk.bold('devkit port — Port & Process Manager'));

  while (true) {
    const action = await select({
      message: 'Choose an action:',
      options: [
        { value: 'list',  label: '📡  List listening ports',     hint: 'all TCP/UDP services' },
        { value: 'find',  label: '🔍  Find port',                hint: 'what is on a specific port' },
        { value: 'kill',  label: '💀  Kill port',                hint: 'stop process on a port' },
        { value: 'check', label: '🔌  Check connectivity',       hint: 'test if host:port is open' },
        { value: 'myip',  label: '🌐  My IP',                    hint: 'local & public IP addresses' },
        { value: 'open',  label: '🚪  Open/expose port',         hint: 'firewall + connectivity info' },
        { value: 'close', label: '🚫  Close/block port',         hint: 'block port on firewall' },
    ...
    switch (action) {
      case 'list': {
        const sp = spinner();
        sp.start('Scanning listening ports...');
        const raw = getListening();
        sp.stop('Done');
      ...
      case 'find': {
        const port = await inlineText('Port number:');
        if (isCancel(port) || !port) break;

        let out;
        try {
          out = execFileSync('lsof', ['-nP', '-i', `:${port}`], { encoding: 'utf-8', timeout: 8000 });
        } catch {
          note(`Nothing found on port ${port}`, 'Port Status');
          break;
        }
        note(out.trim(), `Port ${port}`);
        break;
      }
      ...
      case 'kill': {
        ...
        try {
          ...
          note(`Nothing running on port ${port}`, 'Kill');
          break;
        }

        const pids = out.trim().split('\n').filter(Boolean);
        if (pids.length === 0) { note('Nothing running', 'Kill'); break; }

        if (pids.length === 1) {
          const pid = pids[0];
          let pname = 'unknown';
          try { pname = execFileSync('ps', ['-p', pid, '-o', 'comm='], { encoding: 'utf-8', timeout: 3000 }).trim(); } catch {}
          const ok = await confirm({ message: `Kill ${pname} (PID ${pid})?`, initialValue: false });
          if (!ok) { note('Cancelled', 'Kill'); break; }
          try {
            process.kill(parseInt(pid, 10), 'SIGTERM');
            note(`Killed ${pname} (PID ${pid})`, '✅ Done');
          } catch {
            try { execFileSync('kill', ['-9', pid], { stdio: 'pipe' }); note(`Force killed ${pname} (PID ${pid})`, '✅ Done'); }
            catch (e) { cancel(`Cannot kill: ${e.message}`); }
          }
          break;
        }
        ...
        const procs = pids.map(pid => {
          let name = 'unknown';
          try { name = execFileSync('ps', ['-p', pid, '-o', 'comm='], { encoding: 'utf-8', timeout: 3000 }).trim(); } catch {}
          return { value: pid, label: `${name} (PID ${pid})` };
        });
        procs.push({ value: 'all', label: `⚡ Kill ALL ${procs.length} on port ${port}` });
        procs.push({ value: '__cancel', label: 'Cancel' });

        const sel = await select({ message: `Processes on port ${port}:`, options: procs });
        if (isCancel(sel) || sel === '__cancel') break;

        const targets = sel === 'all' ? pids : [sel];
        let killed = 0;
        for (const pid of targets) {
          try { process.kill(parseInt(pid, 10), 'SIGTERM'); killed++; }
          catch { try { execFileSync('kill', ['-9', pid], { stdio: 'pipe' }); killed++; } catch {} }
        }
        note(`Killed ${killed} process(es) on port ${port}`, '✅ Done');
        break;
      }

      case 'check': {
        const target = await text({ message: 'Host:port (e.g. localhost:8080):', validate: v => v ? undefined : 'Required' });
        if (isCancel(target) || !target) break;
        const parts = target.split(':');
        const host = parts.length >= 2 ? parts.slice(0, -1).join(':') : 'localhost';
        const port = parts.length >= 2 ? parts[parts.length - 1] : target;

        const sp = spinner();
        sp.start(`Checking ${host}:${port}...`);
        try {
          execFileSync('nc', ['-z', '-w', '3', host, port], { encoding: 'utf-8', timeout: 10000 });
          sp.stop('Open');
          note('', `✅  Port ${port} is open on ${host}`);
        } catch (e) {
          if (e.stderr && e.stderr.includes('succeeded')) {
            sp.stop('Open');
            note('', `✅  Port ${port} is open on ${host}`);
            ...
            sp.stop('Closed');
            note(`Cannot reach ${host}:${port}`, '❌  Closed or host down');
          }
        }
        break;
      }
      ...
      case 'myip': {
        const myipLines = [chalk.bold('  IP Addresses:')];
        myipLines.push(chalk.dim('  ───────────────────────────'));
        ...
        for (const iface of ['en0', 'en1', 'en2', 'en3', 'en4', 'en5']) {
          try {
            ...
            if (ip) {
              const name = iface === 'en0' ? 'Wi-Fi' : iface === 'en1' ? 'Ethernet' : iface;
              myipLines.push(`  ${chalk.green('●')} ${chalk.bold(name.padEnd(12))} ${ip}`);
            }
          } catch {}
        }
        for (const iface of ['utun0', 'utun1', 'utun2', 'utun3']) {
          try {
            const raw = execFileSync('ifconfig', [iface], { encoding: 'utf-8', timeout: 5000 });
            const m = raw.match(/inet\s+(\d+\.\d+\.\d+\.\d+)/);
            if (m) myipLines.push(`  ${chalk.yellow('●')} ${chalk.bold('VPN'.padEnd(12))} ${m[1]} (${iface})`);
          } catch {}
        }
        myipLines.push(`  ${chalk.dim('●')} ${chalk.bold('localhost'.padEnd(12))} 127.0.0.1`);

        myipLines.push('');
        myipLines.push(chalk.bold('  Public IP:'));
        try {
          const publicIp = execFileSync('curl', ['-s', '--max-time', '5', 'ifconfig.me'], { encoding: 'utf-8', timeout: 8000 }).trim();
          if (publicIp) myipLines.push(`  ${chalk.cyan('●')} ${publicIp}`);
          else myipLines.push(chalk.yellow('  Could not determine'));
        } catch {
          myipLines.push(chalk.yellow('  Could not determine (check internet connection)'));
        }
      ...
      case 'open': {
        ...
        if (listening) {
          lines.push('✅  Service is listening on this port');
        } else {
          lines.push('⚠️  Nothing is listening on this port');
          lines.push('  Start your service first, then check firewall below.');
        }
        lines.push('');
        ...
        let pfEnabled = false;
        try {
          const pf = execFileSync('pfctl', ['-s', 'info'], { encoding: 'utf-8', timeout: 5000 });
          pfEnabled = pf.includes('Status: Enabled');
        } catch {}
        if (pfEnabled) {
          lines.push(`🔥  pf firewall is ON — allow inbound with:`);
          lines.push(`  sudo pfctl -a "devkit/${port}" -f /dev/stdin <<< "pass in proto tcp from any to any port ${port}"`);
        } else {
          lines.push('✅  pf firewall is off');
        }

        // Check ALF
        try {
          const alf = execFileSync('/usr/libexec/ApplicationFirewall/socketfilterfw', ['--getglobalstate'], { encoding: 'utf-8', timeout: 5000 });
          if (alf.includes('enabled')) {
            lines.push('⚠️  macOS App Firewall is ON (System Settings > Network > Firewall)');
          } else {
            lines.push('✅  macOS App Firewall is off');
          }
        } catch {}

        if (lines.length > 0) note(lines.join('\n'), `Port ${port} — Expose Check`);
        break;
      }

      case 'close': {
        const port = await text({ message: 'Port to block:', validate: v => /^\d+$/.test(v) ? undefined : 'Enter a number' });
        if (isCancel(port) || !port) break;
        ...
        if (pfEnabled) {
          lines.push(`Block inbound on port ${port} with pf:`);
          lines.push(`  sudo pfctl -a "devkit/${port}" -f /dev/stdin <<< "block in proto tcp from any to any port ${port}"`);
        } else {
          lines.push('✅  pf firewall is off — no explicit block needed');
        }
        lines.push('');
        lines.push(`Also stop any service on port ${port}:  devkit port kill ${port}`);
...
const tool = defineTool({
  manifest: { name: 'port', label: '📡  Port Manager', hint: 'list, find, kill ports, firewall' },
  commands,
  execute,
  main,
});
